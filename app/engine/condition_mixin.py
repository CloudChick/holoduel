    ...
    def is_condition_met(self, effect_player: PlayerState, source_card_id, condition):
        match condition["condition"]:
            ...
            case Condition.Condition_DamageSourceBloomLevel:
                ...
                return False
            case Condition.Condition_DamageTargetHasTag:
                condition_tags = condition.get("condition_tags", [])
                target_card = self.take_damage_state.target_card
                if target_card:
                    card_tags = target_card.get("tags", [])
                    return any(tag in card_tags for tag in condition_tags)
                return False
            case Condition.Condition_DamageTargetBloomLevel:
                required_bloom_level = condition.get("bloom_level", 1)
                target_card = self.take_damage_state.target_card
                if target_card and target_card.get("card_type") == "holomem_bloom":
                    return target_card.get("bloom_level", 0) == required_bloom_level
                return False
            ...
            case Condition.Condition_MyHolomemDownedLastOpponentTurnTagged:
                condition_tags = condition.get("condition_tags", [])
                return any(tag in effect_player.holomem_downed_tags_last_opponent_turn for tag in condition_tags)
